...
int in_out(int x, int y, int x1, int y1, int x2, int y2)
{
...
void wa_clip()
{
    ...
    for (int i = 0; i < n_cw; i++)
    {
        ...
        {
            intersection_lineseg(x, y, cw[i].x, cw[i].y, cw[(i + 1) % n_cw].x, cw[(i + 1) % n_cw].y, sp[j].x, sp[j].y, sp[(j + 1) % n_sp].x, sp[(j + 1) % n_sp].y);
            if (x == -1) // or y==-1
                continue;
            ti++;
            tempi[ti][0].x = x;
    ...
    for (int i = 0; i < n_sp; i++)
    {
        ...
        {
            intersection_lineseg(x, y, cw[j].x, cw[j].y, cw[(j + 1) % n_cw].x, cw[(j + 1) % n_cw].y,sp[i].x, sp[i].y, sp[(i + 1) % n_sp].x, sp[(i + 1) % n_sp].y);
            if (x == -1) // or y==-1
                continue;
            ti++;
            tempi[ti][0].x = x;
